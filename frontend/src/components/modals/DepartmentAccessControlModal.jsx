...
// const DepartmentAccessControlModal = ({ open, onClose, projectId }) => {
...
//   if (!open || !isAdminLike) return null;
...
//         ...f,
//         department_can_see_folder: Boolean(f.department_can_see_folder),
//         department_can_view: Boolean(f.department_can_view),
//         department_can_upload: Boolean(f.department_can_upload),
//         department_can_download: Boolean(f.department_can_download),
//         department_can_delete: Boolean(f.department_can_delete),
//       }));

//       setFolders(normalized);
//       setHydrated(true);
//     } catch (err) {
//       console.error("Department access load error", err);
//       toast.error("Failed to load department permissions");
//     } finally {
//       setLoading(false);
//     }
//   };
...
//         children[f.parent_id] = children[f.parent_id] || [];
...
//     roots.forEach((r) => {
//       result.push(r);
//       if (children[r.id]) result.push(...children[r.id]);
//     });
...
//         // Turning OFF Available → disable everything
//         if (
//           key === "department_can_see_folder" &&
//           !updated.department_can_see_folder
//         ) {
//           updated.department_can_view = false;
//           updated.department_can_upload = false;
//           updated.department_can_download = false;
//           updated.department_can_delete = false;
//         }

//         // Enabling any permission requires Available + View
//         if (key !== "department_can_see_folder" && updated[key]) {
//           updated.department_can_see_folder = true;
//           updated.department_can_view = true;
//         }

//         // Turning OFF View disables others (but keeps Available)
//         if (key === "department_can_view" && !updated.department_can_view) {
//           updated.department_can_upload = false;
//           updated.department_can_download = false;
//           updated.department_can_delete = false;
//         }
...
//           department_can_see_folder: f.department_can_see_folder,
//           department_can_view: f.department_can_view,
//           department_can_upload: f.department_can_upload,
//           department_can_download: f.department_can_download,
//           department_can_delete: f.department_can_delete,
//         });
//       }

//       toast.success("Department access updated");
//       await loadFolders(); // force fresh DB state
//       onClose();
//     } catch (err) {
//       console.error("Department save error", err);
//       toast.error("Failed to save department permissions");
//     } finally {
//       setSaving(false);
//     }
//   };
...
//     { key: "department_can_see_folder", label: "Available", icon: Eye },
//     { key: "department_can_view", label: "View", icon: Eye },
//     { key: "department_can_upload", label: "Upload", icon: Upload },
//     { key: "department_can_download", label: "Download", icon: Download },
//     { key: "department_can_delete", label: "Delete", icon: Trash2 },
//   ];
...
//                 Control department permissions per folder
//               </p>
//             </div>
//           </div>
...
//                           type="checkbox"
//                           checked={f[p.key]}
//                           onChange={() => togglePermission(f.id, p.key)}
//                         />
//                       </td>
//                     ))}
//                   </tr>
...
//             onClick={handleSave}
//             disabled={saving}
//             className="px-6 py-2 rounded-xl bg-indigo-600 text-white flex items-center gap-2"
//           >
...
// export default DepartmentAccessControlModal;


// src\components\modals\DepartmentAccessControlModal.jsx
import React, { useEffect, useMemo, useState } from "react";
import {
  X,
  Save,
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4">
        ...
        <div className="relative w-full max-w-xs sm:max-w-sm md:max-w-md bg-white rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden animate-modalSlideIn">
          {/* Decorative Elements */}
          <div className="absolute top-0 right-0 w-32 sm:w-40 h-32 sm:h-40 bg-gradient-to-br from-indigo-400/20 to-blue-500/20 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2" />
          <div className="absolute bottom-0 left-0 w-24 sm:w-32 h-24 sm:h-32 bg-gradient-to-tr from-blue-400/20 to-indigo-500/20 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2" />

          {/* Header */}
          <div className="relative px-5 sm:px-6 md:px-8 pt-6 sm:pt-8 pb-4 sm:pb-6">
            <div className="flex items-start gap-3 sm:gap-4">
              <div className="flex-shrink-0 w-11 h-11 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-xl sm:rounded-2xl bg-gradient-to-br from-indigo-400 to-blue-500 flex items-center justify-center shadow-lg shadow-indigo-500/30">
          ...
          <div className="relative px-5 sm:px-6 md:px-8 pb-5 sm:pb-6">
            <p className="text-sm sm:text-base text-slate-600 leading-relaxed">
              Department Access Control must be opened from within a specific
              project context.
            </p>

            <div className="mt-4 sm:mt-5 flex items-start gap-2.5 sm:gap-3 p-3 sm:p-4 bg-gradient-to-r from-indigo-50 to-blue-50 border border-indigo-200/60 rounded-xl sm:rounded-2xl">
              <div className="flex-shrink-0 w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-lg sm:rounded-xl bg-indigo-100 flex items-center justify-center">
                <Info className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-indigo-600" />
              </div>
              <p className="text-xs sm:text-sm text-indigo-800 leading-relaxed pt-0.5 sm:pt-1">
                Navigate to the <span className="font-semibold">Projects</span>{" "}
                section and select a project to manage department permissions.
              </p>
            </div>
          </div>
  ...
  const togglePermission = (folderId, key) => {
    ...
    setFolders((prev) =>
      prev.map((f) => {
        ...
        if (
          key !== "department_can_see_folder" &&
          (updated.department_can_upload ||
            updated.department_can_download ||
            updated.department_can_delete)
        ) {
          updated.department_can_view = true;
        }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4">
      ...
      <div className="relative w-full max-w-full sm:max-w-xl md:max-w-3xl lg:max-w-4xl xl:max-w-5xl max-h-[95vh] sm:max-h-[90vh] bg-white rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-modalSlideIn">
        {/* Decorative Elements */}
        <div className="absolute top-0 right-0 w-48 sm:w-64 md:w-80 lg:w-96 h-48 sm:h-64 md:h-80 lg:h-96 bg-gradient-to-br from-indigo-400/10 to-blue-500/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none" />
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 text-cyan-300 hidden sm:block" />
                </div>
                <p className="text-xs sm:text-sm text-white/70 mt-0.5 sm:mt-1 font-medium flex items-center gap-1.5 sm:gap-2">
                  ...
                  <span className="line-clamp-1">
                    Manage department permissions for project folders
                  </span>
                </p>
              </div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto bg-gradient-to-b from-slate-50 to-white overscroll-contain">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-16 sm:py-20 md:py-24">
              <div className="relative">
                <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 border-3 sm:border-4 border-indigo-100 rounded-full" />
              ...
              <p className="mt-4 sm:mt-5 md:mt-6 text-sm sm:text-base font-medium text-slate-600">
                Loading department permissions...
              </p>
                ...
                <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-200/60 overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead>
                        <tr className="bg-gradient-to-r from-indigo-900 to-blue-800">
              ...
              <div className="lg:hidden space-y-3 sm:space-y-4">
                {orderedFolders.map((f) => {
                  ...
                  return (
                    <div
                      key={f.id}
                      className="bg-white rounded-xl sm:rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-200 overflow-hidden transition-all duration-200 hover:shadow-xl"
                    >
                      {/* Folder Header */}
                            ...
                            <div className="flex items-center gap-1.5 sm:gap-2 md:gap-2.5">
                              <perm.icon
                                className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${
                                  f[perm.key] ? "text-white" : "text-slate-500"
                                }`}
                              />
                              <span
                                className={`text-xs sm:text-sm font-semibold ${
                                  f[perm.key] ? "text-white" : "text-slate-700"
                                }`}
                              >
                                {perm.label}
                              </span>
